...
@section('content_header')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">{{ __('app.headers.goodReceivesManagement') }}</li>
    </ol>
@stop
...
@section('content')
    <div class="card">
        <div class="card-header">
            <div class="row">
                <h4><i class="fas fa-fw fa-truck-loading mr-2"></i>{{ __('app.headers.goodReceivesManagement') }}</h4>
                <button type="button" class="btn btn-primary ml-auto" id="createButton"
                    onclick="openCreateGoodsReceiveModal()">
...
@section('js')
    ...
    <script>
        ...
        const viewUrl = "{{ route('goodReceives.show', ':id') }}";
        // Delete Data URL
        const deleteUrl = "{{ route('goodReceives.destroy', ':id') }}";
        // Datatable ID
        const dataTableName = 'items_list_table';
        // Table Columns List
        const dataTableColumns = ["id", "good_receive_number", "supplier_name", "date", "time", "total_text"];
        ...
        const openPurchaseOrder = (data) => {
            const documentUrl =
                "{{ route('documents.getPdf', ['type' => 'goodReceive', 'id' => ':id', 'action' => 'view']) }}";
