...
@section('content_header')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">{{ __('app.headers.salesReturnsManagement') }}</li>
    </ol>
@stop
...
@section('content')
    <div class="card">
        <div class="card-header">
            <div class="row">
                <h4><i class="fas fa-fw fa-history mr-2"></i>{{ __('app.headers.salesReturnsManagement') }}</h4>
                <button type="button" class="btn btn-primary ml-auto" id="createButton"
                    onclick="openCreateSalesReturnModal()">
...
@section('js')
    ...
    <script>
        // Load Data URL
        const indexUrl = "{{ route('salesReturns.index') }}";
        // View Selected Data URL
        const viewUrl = "{{ route('salesReturns.show', ':id') }}";
        // Delete Data URL
        const deleteUrl = "{{ route('salesReturns.destroy', ':id') }}";
        // Datatable ID
        const dataTableName = 'items_list_table';
        // Table Columns List
        const dataTableColumns = ["id", "sales_return_number", "prescription_number", "date", "time", "total_text"];
        ...
        const openPurchaseOrder = (data) => {
            const documentUrl =
                "{{ route('documents.getPdf', ['type' => 'salesReturn', 'id' => ':id', 'action' => 'view']) }}";
